...
def retrieve_error_pattern(question, response, fail_memories, revision_memory, retriever, threshold=0.7):
    input_text = question + " " + response
    
    with torch.no_grad():
        query_vec = retriever.encode(input_text, task="text-matching")
    ...
    #    return ""
    #else:
    #    return {"type": revision_memory[idx.item()]["type"], "format": revision_memory[idx.item()]["format"]}
    return max_sim, {"type": revision_memory[idx.item()]["type"], "format": revision_memory[idx.item()]["format"]}
